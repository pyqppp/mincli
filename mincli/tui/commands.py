...
CMD_SPEC: tuple[Cmd, ...] = (
    ...
    Cmd("import", "导入文件/网页/图片（图片转为待发送图片）", args="<文件路径或URL> [...]",
        group="多模态",
        children=(
            Cmd("clear", "清除已导入、待发送的内容", aliases=("c",)),
        )),
    Cmd("files", "管理已上传的 Files API 图片文件", group="多模态",
        children=(
            Cmd("list", "列出已上传的图片文件", aliases=("ls",)),
            Cmd("delete", "删除指定图片文件", args="<ID>", aliases=("rm", "del")),
        )),
    ...
    Cmd("set", "修改运行配置", group="配置",
        children=(
            ...
            Cmd("detail", "设置图片清晰度（low 省 token，original 最清晰）",
                args="<low|auto|high|original>",
                children=(
                    Cmd("low", "最低清晰度，最省 token"),
                    Cmd("auto", "自动选择（默认）"),
                    Cmd("high", "高清晰度"),
                    Cmd("original", "原图，最清晰"),
                )),
