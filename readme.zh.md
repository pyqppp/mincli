...
- **命令补全** — 输入 `/` 弹出命令列表（每个候选一行：用法 + 灰色说明）；支持二级命令（`/set thinking`）与三级取值（`on|off`）以及运行时候选（对话树编号、工作流名、MCP server 名）；`Tab` 循环（高亮项自动滚入可视区）、`Shift+Tab` 反向、`Enter` 补全；命令写完没有下一级时自动显示用法帮助
...
- 🖼️ **多模态（图片理解）** — `/import 图片` 附带图片提问（本地文件自动上传 Files API，跨轮复用 file_id；`/set detail low` 改走内联让 detail 真正生效；上传失败自动回退 base64 内联；导入即按官方公式给出 token 估算；`/files` 可列表/查询/删除/清理远端文件；图片理解仅 `deepseek-flash` 支持，其他模型会提示切换；支持一次导入多个文件/图片）
- 📊 **实时用量状态条** — 输入栏下方两分栏实时显示：缓存命中率、账户余额、下一次输入 token 与预计价格（用 API 真实 `usage` 推算，与对话结束显示的输入/输出同口径；价格/峰谷时段可覆盖，图片 token 兜底值也可配置）
...
| `Enter` | 发送消息 |
| `Ctrl+J` / `Alt+Enter` | 换行 |
| `Tab` / `Shift+Tab` | 命令补全：循环切换补全候选 / 反向循环 |
...
| `/files list [N]` | 列出最近上传的文件（默认 20 条，**最新在前**，带序号/大小/创建时间/过期/本页合计与官方配额） |
| `/files info <ID\|序号>` | 查询单个已上传文件的信息 |
| `/files delete <ID\|序号>` | 删除一个已上传的图片文件，并同步清理引用它的对话树 |
| `/files clean` | 清理不再被任何对话树引用的远端文件（需确认） |
| `/<节点ID>`（如 `/a3`） | 直接跳转到指定节点 |
...
在输入框输入 `/` 即可看到命令列表：每个候选一行（用法 + 灰色说明），继续输入过滤候选；`Tab` 循环、`Shift+Tab` 反向、`Enter` 补全。补全支持多级：`/set` → `/set thinking` → `/set thinking on`，以及运行时才知道的候选（`/tree <编号>`、`/wf show <名>`、`/mcp remove <名称>`）。命令写完且没有下一级时，输入框上方显示该命令的用法与说明；要执行命令再按一次 `Enter`。
...
- 导入的文件会显示在输入框下方状态栏（`📎 已导入 N 个文件：前2个文件名…`，含图片的还会带上「图片约 N tokens」估算）；鼠标悬停该状态栏会弹出完整文件名列表（每张图片单独标注估算 token），移开鼠标自动消失。
...
### 传图机制（Files API 优先，detail=low 走内联）

1. **上传一次，跨轮复用**：本地图片首次发送时上传到 DeepSeek Files API，得到 `file_id`（`file-api-...`），节点持久化该 ID；后续所有请求（含历史重放、分支追问）都通过 `file` 内容块引用，请求体极小、序列化稳定，**不破坏前缀缓存**。历史节点里缺 `file_id` 的图片会在下一次发送前自动补传。
2. **detail=low 改为内联**：`file` 内容块**没有** `detail` 字段（官方明确「通过 `file_id` 传图时 `detail` 被忽略」），所以 `/set detail low` 时本地图片改走 base64 内联的 `image_url`，让「省 token」真正生效（请求级内联预算 24MiB，按体积从小到大装，给 48MiB 请求体留出文本余量；超出预算或单图 >32MiB 的仍上传并明确提示该图 detail 被忽略）。`auto` / `high` / `original` 三档与上传后的处理一致，仍优先上传复用。
3. **自动回退**：上传失败时自动改用 base64 内联发送（内联单图 ≤32MiB、请求体 ≤48MiB 预检）；本地图片最大可到 64MiB（Files API 单图上限），超过 32MiB 且上传失败时会明确报错。`file_id` 失效（过期/被删）时自动降级为内联重试一次。
4. **文件管理**：`/files list [N]` 查看最近上传的文件（默认 20 条，**最新在前**，带序号、本页合计与官方配额），`/files info <ID|序号>` 查询单个文件，`/files delete <ID|序号>` 删除，`/files clean` 清理不再被任何对话树引用的远端文件（需确认）。删除文件或对话节点时会同步清掉对话树里的失效引用。
5. **外部 URL**：直接传 `http(s)` 链接（≤8192 字符，API 需在 60 秒内下载完成）。

### 图片 token 与成本（按官方预处理公式估算）

- 官方预处理流程：`detail=low` 先把图片缩到单边 ≤512px；总像素 <约 544×544 的图片按长宽比放大；宽高各自对齐到 patch 14 的整数倍、再按 3 倍下采样得到 token 网格；单图 token = `n_h * (n_w + 1) + 2`，**上限 1024**。
- mincli 按这个公式（参数取自 `deepseek-ai/DeepSeek-V4.1-Flash` 的 `vision_config`：`patch_size=14`、`downsample_ratio=3`、`max_image_tokens=1024`、`min_pixels=295936`）用图片实际尺寸逐张换算，结果与官方文档给的锚点完全吻合：2000×2000 与 5000×5000 同为 994 token；544×544 及更小的图同为 184 token；上限 1024 对应缩放后约 1300×1300。
- 估算值会在 `/import` 通知、状态栏与悬停弹窗里显示（`图片约 N tokens`）；尺寸未知（外链 URL）时退回 `~/.mincli/pricing.json` 的 `image_tokens`（默认 1024，即官方上限）。实际计费仍以接口返回的 `usage` 为准。
- **实机实测（2026-09，deepseek-flash）**：8×8 小图与 2000×2000 大图分别用 `detail=low`（内联）和 `auto`（上传 file_id）各发一次，以「纯文本轮」为基线做差：小图两档都是 195 input tokens（含约 11 token 文本，即图片 184）；大图 `low` 是 195、`auto` 是 1005（图片 994）。与上面的公式估算（184 / 994）一致，也验证了 `detail=low` 确实省约 5.4 倍。
...
- `image_tokens` 只在图片尺寸未知（外链 URL）时作为估算兜底，默认 1024（官方单图上限）；本地图片按官方预处理公式精确换算，不读这个值。
- 可用 `mincli info` 查看当前生效的定价配置文件与图片 token 兜底值。
...
│   │   ├── chat.tcss        # TUI 样式
│   │   ├── commands.py      # 斜杠命令规格（补全候选 / /help / 用法提示同源）
...
│       ├── images.py        # 图片附件：格式嗅探/尺寸解析/base64/官方公式 token 估算
│       ├── files.py         # Files API 客户端（上传/倒序分页列表/查询/删除）
│       └── thinking.py      # 审计系统提示词
│
...
**Q：图片 token 消耗大吗？**  
A：按官方预处理公式换算：小图（≤544×544）约 184 token，800×600 约 317，1920×1080 约 968，超大图触顶 994（上限 1024）。`/set detail low` 会先把图片缩到单边 512px，大图可降到约 184 token（约省 80%，实机实测 2000×2000：low 184 / auto 994），代价是该图走 base64 内联、不再复用 `file_id`。导入时状态栏与悬停弹窗会显示估算值；图片部分不参与前缀缓存（按未命中价计费），文本前缀可正常缓存。具体以接口返回 `usage` 为准。
